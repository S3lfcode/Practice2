...
fun main() {
   var persons = listOf(
    ...
    persons_S.sortByAverageGrade(true)
...
fun List<Student>.sortByAverageGrade(exeptExtramular: Boolean) {
    this.sortedByDescending{it.averageGrade}
    if(exeptExtramular)
    {
        this.forEach{if (!it.extramular)
            println("Имя: ${it.name} | Средняя оценка: ${it.averageGrade}") }
    }
    else
    {
        this.forEach{ println("Имя: ${it.name} | Средняя оценка: ${it.averageGrade}") }
    }
}
